...
import { ConvertToPascalCase, RemoveSpaces } from "./CommonUtils";
...
export default class ScrollService {
  ...
  scrollToHireMe = () => {
    this.scrollToNext("ContactMe");
  };

  scrollToNext = (nextScreen) => {
    console.log("scrollTo ", nextScreen);

    for (let screen of PORTFOLIO_SCREENS) {
      let screenFromDOM = document.getElementById(screen.screen_name);

      var found = nextScreen === RemoveSpaces(ConvertToPascalCase(screen.screen_name));

      if (found) {
        if (!screenFromDOM) return;
        screenFromDOM.scrollIntoView({ behavior: "smooth" });
        break;
      }
    }
  };
